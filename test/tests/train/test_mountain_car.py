...
def test_mountain_car_mpi():
    ...
    cmd += f"-e MountainCar --clobber --num-timesteps {num_timesteps}"

    run_training(cmd)

    cmd  = f"python train_baseline.py "
    cmd += f"-e MountainCar --test --test-explore --num-test-runs 5 "
...
def test_mountain_car_continous_mpi():
    ...
    cmd  = f"mpirun -n 4 python train_baseline.py "
    cmd += f"-e MountainCarContinuous --clobber --num-timesteps {num_timesteps}"

    run_training(cmd)

    cmd  = f"python train_baseline.py "
    cmd += f"-e MountainCarContinuous --test --test-explore --num-test-runs 5 "
    cmd += f"--save-test-scores"
