...
class ZooPixelNetwork(PPOConv2dNetwork):
    ...
    def __init__(self,
        ...
        try:
            iter(out_dim)
            if len(out_dim) > 1:
                msg  = "ERROR: out_dim should be a single element, but "
                msg += "received and iterable of length {len(out_dim)}."
                rank_print(msg)
                comm.Abort()
        ...
        super(ZooPixelNetwork, self).__init__(
            out_dim = out_dim,
            **kw_args)
